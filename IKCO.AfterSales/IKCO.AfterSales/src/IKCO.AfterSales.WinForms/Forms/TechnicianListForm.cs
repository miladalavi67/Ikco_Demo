...
{
    public class TechnicianListForm : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            _grid.Columns.Add(UiHelper.TextColumn("کد پرسنلی", "PersonnelCode", 80,
                null, DataGridViewContentAlignment.MiddleCenter));
            _grid.Columns.Add(UiHelper.TextColumn("تخصص", "Specialty", 140));
            _grid.Columns.Add(UiHelper.TextColumn("نرخ ساعتی (ریال)", "HourlyRate", 110, "#,##0"));
            _grid.Columns.Add(UiHelper.TextColumn("فعال", "IsActive", 60,
                null, DataGridViewContentAlignment.MiddleCenter));
        ...
        {
            get { return _grid.CurrentRow == null ? null : _grid.CurrentRow.DataBoundItem as Technician; }
        }

        private void BtnNew_Click(object sender, EventArgs e)
        {
            using (var form = new TechnicianEditForm(null))
            { if (form.ShowDialog(this) == DialogResult.OK) LoadData(); }
        }

        private void BtnEdit_Click(object sender, EventArgs e)
        {
            var technician = Selected;
            if (technician == null) { UiHelper.Warn(Messages.NoRowSelected); return; }

            using (var form = new TechnicianEditForm(technician.TechnicianId))
            { if (form.ShowDialog(this) == DialogResult.OK) LoadData(); }
        }
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                _repository.Delete(technician.TechnicianId);
